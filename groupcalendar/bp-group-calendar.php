...
/**
 * Register the BP Group Calendar widgets.
 * @version 1.0, 17/10/2022
 * @author lenasterg
 */
function bp_group_calendar_register_widgets() {
...
/**
 *
 * @global type $bp_group_calendar_current_version
 * @version 2.0
 */
function bp_group_calendar_make_current() {

	global $bp_group_calendar_current_version;
...
if ( class_exists( 'BP_Group_Extension' ) ) {
	class BP_Group_Calendar_Extension extends BP_Group_Extension {
		...
		function display( $group_id = null ) {
			...
			} elseif ( isset( $date['year'] ) && ! isset( $date['month'] ) && ! isset( $date['day'] ) ) {
...
function bp_group_calendar_event_save() {
	...
	if ( isset( $_POST['create-event'] ) ) {
		...
		if ( $tmp_date && strtotime( $_POST['event-date'] ) && ( - 1 !== strtotime( $_POST['event-date'] ) ) ) {
		...
		if ( isset( $_POST['event-id'] ) ) {
			...
			if ( 'limited' === $calendar_capabilities ) {
			...
			$query = $wpdb->prepare(
				'UPDATE ' . $wpdb->base_prefix . 'bp_groups_calendars
			...
			$query = $wpdb->prepare(
				'SELECT * FROM ' . $wpdb->base_prefix . "bp_groups_calendars WHERE group_id='%d' AND user_id='%d' AND event_time='%s' AND event_title='%s' AND event_description='%s' AND event_location='%s'",
				$group_id,
				$current_user->ID,
				$event_date,
				$event_title,
			...
			$query = $wpdb->prepare(
				'INSERT INTO ' . $wpdb->base_prefix . 'bp_groups_calendars
...
function bp_group_calendar_event_email( $new, $event_id, $event_date, $event_title, $event_all_day ) {
	...
	$email_subject     = sprintf( __( '%1$s created a new event for your group %2$s', 'groupcalendar' ), bp_core_get_userlink( $bp->loggedin_user->id, true ), $bp->groups->current_group->name );
...
function bp_group_calendar_highlighted_events( $group_id, $date = '' ) {
	...
	if ( $date ) {
		$start_date = gmdate( 'Y-m-d H:i:s', strtotime( gmdate( 'Y-m-', strtotime( $date ) ) . '01 00:00:00' ) - ( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ) );
		$end_date   = gmdate( 'Y-m-d H:i:s', strtotime( '+1 month', strtotime( $date ) ) - ( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ) );
	} else {
		$start_date = gmdate( 'Y-m-d H:i:s', strtotime( gmdate( 'Y-m-' ) . '01 00:00:00' ) - ( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ) );
		$end_date   = gmdate( 'Y-m-d H:i:s', strtotime( '+1 month', strtotime( $start_date ) ) );
	}
	...
	if ( $events ) {
		...
		foreach ( $events as $event ) {
			...
			$highlighted_events[] = gmdate( 'Y-m-d', $tmp_date );
		}
...
function bp_group_calendar_list_events( $group_id, $range, $calendar_capabilities, $date = '', $show_all = 0 ) {
	...
	if ( 'all' === $range ) {
		...
		$start_date    = gmdate( 'Y-m-d H:i:s', strtotime( $date ) - ( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ) );
		$end_date      = gmdate( 'Y-m-d H:i:s', strtotime( gmdate( 'Y-m-d', strtotime( $date ) ) . ' 23:59:59' ) - ( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ) );
		$filter        = " WHERE group_id = $group_id AND event_time >= '$start_date' AND event_time <= '$end_date'";
		$empty_message = __( 'There are no events scheduled for this day', 'groupcalendar' );
		$date_format   = get_option( 'time_format' );

	} elseif ( 'upcoming' === $range ) {

		$filter        = " WHERE group_id = $group_id AND event_time >= '" . gmdate( 'Y-m-d H:i:s' ) . "'";
		$empty_message = __( 'There are no upcoming events', 'groupcalendar' );

	} elseif ( 'mine' === $range ) {

		$filter        = " WHERE group_id = $group_id AND event_time >= '" . gmdate( 'Y-m-d H:i:s' ) . "' AND user_id = " . $current_user->ID;
			...
			<tr>
				...
				<td class="cal-right">
					<h5 class="events-title"><?php _e( 'Events For', 'groupcalendar' ); ?> <?php echo date_i18n( get_option( 'date_format' ), strtotime( $day ) ); ?>
						:</h5>
					<?php bp_group_calendar_list_events( $bp->groups->current_group->id, 'day', $calendar_capabilities, $day ); ?>
				</td>
			</tr>
		</table>
	</div>
...
function bp_group_calendar_widget_month( $date ) {
	...
	$first_day              = $cal->year . '-' . $cal->month . '-01';
	$cal->highlighted_dates = bp_group_calendar_highlighted_events( $bp->groups->current_group->id, $first_day );
	$previous_month         = $url . gmdate( '/Y/m/', strtotime( '-1 month', strtotime( $first_day ) ) );
	$next_month             = $url . gmdate( '/Y/m/', strtotime( '+1 month', strtotime( $first_day ) ) );
	$this_year              = $url . gmdate( '/Y/', strtotime( $first_day ) );
	?>
	<div class="bp-widget">
			...
			<tr>
				...
				<td class="cal-right">
					<h5 class="events-title"><?php _e( 'Events For', 'groupcalendar' ); ?> <?php echo date_i18n( 'F Y', strtotime( $first_day ) ); ?>
						:</h5>
					<?php bp_group_calendar_list_events( $bp->groups->current_group->id, 'month', $calendar_capabilities, $first_day ); ?>
...
function bp_group_calendar_widget_year( $date ) {
	...
	$previous_year = $url . gmdate( '/Y/', strtotime( '-1 year', strtotime( $first_day ) ) );
	$next_year     = $url . gmdate( '/Y/', strtotime( '+1 year', strtotime( $first_day ) ) );
	...
	<div class="bp-widget">
		...
		for ( $i = 1; $i <= 12; $i ++ ) {
			...
			$year       = gmdate( 'Y', $next_stamp );
			$month      = gmdate( 'm', $next_stamp );
		}
		?>
		<div class="clear"></div>
	</div>
...
function bp_group_calendar_widget_event_display( $event_id ) {
	...
	if ( 'full' === $calendar_capabilities || ( 'limited' === $calendar_capabilities && $current_user->ID === $event->user_id ) ) {
	...
	<div class="bp-widget">
		...
		<span
			class="activity">
			<?php
			echo bgc_date_display( $event->event_time, get_option( 'date_format' ) . __( ' \a\t ', 'groupcalendar' ) . get_option( 'time_format' ), $event->event_all_day );
			?>
			</span>
	...
	<div class="bp-widget">
		...
		<form action="<?php echo $url; ?>" name="add-event-form" id="add-event-form" class="standard-form" method="post"
			...
			<label for="event-all-day">
					<?php _e( 'No event time', 'groupcalendar' ); ?>
...
function bp_group_calendar_widget_edit_event( $event_id = false ) {
	...
	if ( $event_id ) { //load from DB
		...
		if ( 'limited' === $calendar_capabilities && $current_user->ID !== $event->user_id ) {
			bp_core_add_message( __( "You don't have permission to edit that event", 'groupcalendar' ), 'error' );
		...
		if ( $tmp_date ) {
			...
			$event_minute = gmdate( 'i', $tmp_date );
		...
		$event_map         = ( 1 === $event->event_map ) ? ' checked="checked"' : '';
	...
	function widget( $args, $instance ) {
		...
		if ( $events ) {
			...
			foreach ( $events as $event ) {
				...
				$events_list .= stripslashes( $event->name ) . '<br /><a href="' . $url . '" title="' . __( 'View Event', 'groupcalendar' ) . '">' . bgc_date_display( $event->event_time, false, $event->event_all_day ) . ': ' . stripslashes( $event->event_title ) . '</a>';
